...
final class HTMega_Addons_Elementor {
   ...
   /**
    ...
    public function admin_promo_notice(){
        ...
        if( isset( self::$template_info['notices']['banner'][0]['status'] ) ){
            
            if( self::$template_info['notices']['banner'][0]['status'] == 0 ){
                return;
            }
        }else{
            return;
        ...
        // Fetch data
        $bannerLink = self::$template_info['notices']['banner'][0]['bannerlink'] ? self::$template_info['notices']['banner'][0]['bannerlink'] : '#';
        $bannerTitle = self::$template_info['notices']['banner'][0]['title'] ? self::$template_info['notices']['banner'][0]['title'] : esc_html__('Promo Banner','htmega-addons');
        $bannerDescription = self::$template_info['notices']['banner'][0]['description'] ? self::$template_info['notices']['banner'][0]['description'] : '';
        $bannerImage = self::$template_info['notices']['banner'][0]['bannerimage'] ? '<img src="'.esc_url(self::$template_info['notices']['banner'][0]['bannerimage']).'" alt="'.esc_attr($bannerTitle).'"/>' : '';

        $banner['image'] = $bannerImage;
        $banner['url'] = $bannerLink;
        \HasTech_Notices::set_notice(
            [
                'id'          => 'htmega-promotional-banner',
                'type'        => 'info',
                'dismissible' => true,
