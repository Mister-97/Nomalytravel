...
@php
    ...
    $each     = (float) $group['price'];
    $deliv    = implode(', ', $group['delivery_methods'] ?? []) ?: 'E-Ticket';
@endphp

<div class="stc-header">
    <div class="container d-flex justify-content-between align-items-center">
        <h2><i class="fas fa-ticket-alt me-2"></i>Complete Your Ticket Order</h2>
        <a href="{{ route('tickets.event', ['id' => $event['id']]) }}" class="stc-back"><i class="fas fa-arrow-left me-1"></i> Back to listings</a>
    </div>
</div>
        ...
        <div class="row g-4 justify-content-center">
            ...
            <div class="col-lg-7">
                ...
                <div class="stc-policy">
                    <h6><i class="fas fa-info-circle me-1"></i> All Sales Final</h6>
                    <p>Ticket orders are <strong>non-refundable</strong> once confirmed, unless the event is cancelled and not rescheduled. Tickets are delivered by {{ $deliv }}.</p>
                </div>

                <div class="stc-form-card">

                    <div class="stc-section-label"><i class="fas fa-user me-1" style="color:var(--gold)"></i>Buyer Information</div>
                    <div class="row g-3">
                        <div class="col-6">
                            <div class="stc-field">
                    ...
                    <div class="stc-field">
                        <label>Email Address (tickets are delivered here) *</label>
                    ...
                    <label class="stc-agree">
                        ...
                        <span>I agree to the <a href="/terms" target="_blank">Terms &amp; Conditions</a> and understand all ticket sales are final.</span>
                    </label>
                    ...
                    <button class="stc-pay-btn" id="stc-pay-btn" onclick="stcSubmit()">
                        ...
                        Pay {{ $currency }} {{ number_format($total, 2) }} &amp; Confirm Order
                    </button>
            ...
            <div class="col-lg-4">
                <div class="stc-summary" style="position:sticky;top:72px;">
                    <div class="stc-summary-title">Order Summary</div>
                    <div class="stc-summary-hotel">{{ $event['name'] }}</div>
                    <div class="stc-summary-dates">
                        <i class="fas fa-map-marker-alt me-1" style="color:var(--gold);"></i>{{ $venue['name'] ?? '' }}{{ !empty($venue['city']) ? ', '.$venue['city'] : '' }}<br>
                        <i class="fas fa-calendar me-1" style="color:var(--gold);"></i>{{ $when }}
                    </div>
                    <div class="stc-row"><span class="lbl">Section</span><span class="val">{{ $group['section'] ?: 'General Admission' }}</span></div>
                    @if (!empty($group['row']))
                    <div class="stc-row"><span class="lbl">Row</span><span class="val">{{ $group['row'] }}</span></div>
                    @endif
                    <div class="stc-row"><span class="lbl">Quantity</span><span class="val">{{ $quantity }}</span></div>
                    <div class="stc-row"><span class="lbl">Price per ticket</span><span class="val">{{ $currency }} {{ number_format($each, 2) }}</span></div>
                    <div class="stc-row"><span class="lbl">Delivery</span><span class="val">{{ $deliv }}</span></div>
...
var _stcEventId  = {{ (int) $event['id'] }};
var _stcGroupId  = {{ (int) $group['id'] }};
var _stcQty      = {{ (int) $quantity }};
...
async function stcSubmit() {
    ...
    if (!fn || !ln || !em || !ph) { stcSetMsg('Please fill in all buyer details.'); return; }
    ...
    try {
        var piRes = await fetch('{{ route('tickets.payment.intent') }}', {
            method: 'POST',
            headers: { 'Content-Type':'application/json', 'X-CSRF-TOKEN': _csrf, 'X-Requested-With':'XMLHttpRequest' },
            body: JSON.stringify({ event_id: _stcEventId, ticket_group_id: _stcGroupId, quantity: _stcQty })
        });
        ...
        form.method = 'POST';
        form.action = '{{ route('tickets.reserve') }}';
        var fields = {
            _token: _csrf,
            event_id: _stcEventId,
            ticket_group_id: _stcGroupId,
            quantity: _stcQty,
