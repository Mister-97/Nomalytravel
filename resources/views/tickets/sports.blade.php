...
<div class="container py-5">
    <div class="nm-cat-tabs">
        <a href="{{ url('/sports') }}" class="nm-cat-tab {{ !request('keyword') || request('keyword')=='sports' ? 'active' : '' }}">All Sports</a>
        ...
        <a href="{{ url('/sports?keyword=MLS') }}" class="nm-cat-tab {{ request('keyword')=='MLS' ? 'active' : '' }}">⚽ Soccer</a>
        <a href="{{ url('/sports?keyword=UFC') }}" class="nm-cat-tab {{ request('keyword')=='UFC' ? 'active' : '' }}">🥊 UFC</a>
    </div>
    ...
    @elseif(count($events) > 0)
        <p class="text-muted mb-3">
            ...
            @if(request('keyword') && request('keyword') !== 'sports') &bull; <strong>{{ request('keyword') }}</strong>@endif
            &nbsp;<a href="{{ url('/sports') }}" class="text-muted" style="font-size:12px;">(clear)</a>
        </p>
        <div class="nm-events-grid">
            ...
            @php
                ...
                $category  = $event['category']['name'] ?? 'Sports';
                $ticketUrl = $event['url'] ?? '#';
                $icon      = '🏟';
                if(str_contains(strtolower($category),'basketball')) $icon='🏀';
                elseif(str_contains(strtolower($category),'football')) $icon='🏈';
                elseif(str_contains(strtolower($category),'baseball')) $icon='⚾';
                elseif(str_contains(strtolower($category),'hockey')) $icon='🏒';
                elseif(str_contains(strtolower($category),'soccer')) $icon='⚽';
                elseif(str_contains(strtolower($category),'ufc') || str_contains(strtolower($category),'mma')) $icon='🥊';
                elseif(str_contains(strtolower($category),'golf')) $icon='⛳';
                elseif(str_contains(strtolower($category),'tennis')) $icon='🎾';
            @endphp
            <div class="nm-event-card">
                @if(!empty($event['image']))
                <img class="nm-event-img" src="{{ $event['image'] }}" alt="{{ $event['name'] }}" loading="lazy" style="width:100%;height:180px;object-fit:cover;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
                <div class="nm-event-img-placeholder" style="display:none"><i class="fas {{ $icon }} fa-3x" style="color:rgba(255,255,255,0.4)"></i></div>
                @else
                <div class="nm-event-img-placeholder"><i class="fas {{ $icon }} fa-3x" style="color:rgba(255,255,255,0.4)"></i></div>
                @endif
