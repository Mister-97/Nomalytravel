                            ...
                            <div class="booking-form-main">
                                ...
                                <div class="booking-journey-section">
                                    <div class="booking-journey-header">
                                        ...
                                        @php
                                            $depAt = $slice['segments'][0]['departing_at'];
                                            $arrAt = $slice['segments'][count($slice['segments'])-1]['arriving_at'];
                                            $depIata = $slice['segments'][0]['origin']['iata_code'] ?? '';
                                            $arrIata = $slice['segments'][count($slice['segments'])-1]['destination']['iata_code'] ?? '';
                                            $departureTime = \Carbon\Carbon::parse($depAt);
                                            $arrivalTime = \Carbon\Carbon::parse($arrAt);
                                            $duration = $departureTime->diffForHumans($arrivalTime, ['parts' => 2]);
                                            $isNonStop = count($slice['segments']) === 1;
                                            // Format times as 12-hour AM/PM with timezone
                                            $tzMap = ['JFK'=>'America/New_York','LGA'=>'America/New_York','EWR'=>'America/New_York','LAX'=>'America/Los_Angeles','SFO'=>'America/Los_Angeles','ORD'=>'America/Chicago','MDW'=>'America/Chicago','DFW'=>'America/Chicago','DAL'=>'America/Chicago','MIA'=>'America/New_York','FLL'=>'America/New_York','ATL'=>'America/New_York','BOS'=>'America/New_York','SEA'=>'America/Los_Angeles','DEN'=>'America/Denver','PHX'=>'America/Phoenix','LAS'=>'America/Los_Angeles','MSP'=>'America/Chicago','DTW'=>'America/New_York','CLT'=>'America/New_York','PHL'=>'America/New_York','BWI'=>'America/New_York','IAD'=>'America/New_York','DCA'=>'America/New_York','HOU'=>'America/Chicago','IAH'=>'America/Chicago','SLC'=>'America/Denver','PDX'=>'America/Los_Angeles','SAN'=>'America/Los_Angeles','OAK'=>'America/Los_Angeles','LHR'=>'Europe/London','LGW'=>'Europe/London','CDG'=>'Europe/Paris','AMS'=>'Europe/Amsterdam','FRA'=>'Europe/Berlin','MAD'=>'Europe/Madrid','BCN'=>'Europe/Madrid','FCO'=>'Europe/Rome','MXP'=>'Europe/Rome','ZRH'=>'Europe/Zurich','DXB'=>'Asia/Dubai','DOH'=>'Asia/Qatar','SIN'=>'Asia/Singapore','BKK'=>'Asia/Bangkok','SYD'=>'Australia/Sydney','NRT'=>'Asia/Tokyo','HND'=>'Asia/Tokyo','ICN'=>'Asia/Seoul','PEK'=>'Asia/Shanghai','PVG'=>'Asia/Shanghai','HKG'=>'Asia/Hong_Kong','YYZ'=>'America/Toronto','YVR'=>'America/Vancouver','MEX'=>'America/Mexico_City','GRU'=>'America/Sao_Paulo','EZE'=>'America/Argentina/Buenos_Aires'];
                                            $depTz = $tzMap[$depIata] ?? null;
                                            $arrTz = $tzMap[$arrIata] ?? null;
                                            $depFormatted = $depTz ? $departureTime->setTimezone($depTz)->format('g:i A T') : $departureTime->format('g:i A');
                                            $arrFormatted = $arrTz ? $arrivalTime->setTimezone($arrTz)->format('g:i A T') : $arrivalTime->format('g:i A');
                                        @endphp
                                        <div class="booking-journey-timeline">
                                            <!-- Departure -->
                                            <div class="timeline-item departure">
                                                <div class="time">{{ $departureTime->format('D, j M Y') }}, {{ $depFormatted }}</div>
                                            ...
                                            <div class="timeline-item arrival">
                                                <div class="time">{{ $arrivalTime->format('D, j M Y') }}, {{ $arrFormatted }}</div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function updateServiceBreakdown() {
                ...
                const initialTotal = parseFloat('{{ $totalWithServiceFee ?? 0 }}');
