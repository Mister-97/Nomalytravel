...
// Tickets (must be before wildcard slug/module routes)
...
Route::get('/api/airports', function(\Illuminate\Http\Request $request) {
    ...
    try {
        // Read live Duffel key from CMS widget (same source as DuffelService)
        $apiKey = \DB::table('widgets_data')->where('widget_id', 29)->value('extra_field_1')
                  ?: env('DUFFEL_API_KEY');

        $client = new \GuzzleHttp\Client(['timeout' => 5]);
        $response = $client->get('https://api.duffel.com/places/suggestions', [
            'headers' => [
                'Authorization'  => 'Bearer ' . $apiKey,
                'Duffel-Version' => 'v2',
                'Accept'         => 'application/json',
            ],
            'query' => ['query' => $q, 'limit' => 12],
        ]);

        $body = json_decode($response->getBody()->getContents(), true);
        $suggestions = $body['data'] ?? [];

        // Separate cities and airports so we can show grouped results
        $cities    = [];
        $airports  = [];
        foreach ($suggestions as $s) {
            if (empty($s['iata_code'])) continue;
            if ($s['type'] === 'city') {
                $cities[$s['iata_code']] = $s['name'] ?? '';
            } elseif ($s['type'] === 'airport') {
                $airports[] = [
                    'code'    => $s['iata_code'],
                    'name'    => $s['name'] ?? '',
                    'city'    => $s['city_name'] ?? ($s['name'] ?? ''),
        ...
        // If no city groupings matched, just return airports flat
        if (empty($results)) {
            $results = $airports;
        }

        return response()->json(array_slice($results, 0, 14));

    } catch (\Exception $e) {
        \Log::error('Duffel Places API error: ' . $e->getMessage());
...
// Flight booking routes (no auth required - supports guest checkout)
...
// ════════════════════════════════════════
// Homepage AJAX Search API Routes
// ════════════════════════════════════════

// Flights AJAX search (returns JSON when called with XMLHttpRequest header)
Route::get('/api/home/flights', [App\Http\Controllers\DuffelController::class, 'search'])->name('api.home.flights');
...
Route::post('/api/flights/payment-intent', function(Illuminate\Http\Request $request) {
    ...
    try {
        $intent = PaymentIntent::create([
            ...
            'metadata'    => ['offer_id' => $request->input('offer_id','')],
        ]);
...
// Save flight booking after Stripe payment confirmed
Route::post('/api/flights/book', function(Illuminate\Http\Request $request) {
    try {
        $booking = new App\Models\FlightBooking();
        $booking->offer_id          = $request->input('offer_id','');
        $booking->user_id           = auth()->id();
        $booking->origin_code       = $request->input('from','');
        $booking->destination_code  = $request->input('to','');
        $booking->departure_date    = $request->input('depart_date') ? \Carbon\Carbon::parse($request->input('depart_date')) : now();
        $booking->adults            = (int)$request->input('adults',1);
        $booking->total_amount      = (float)$request->input('price',0);
        $booking->currency          = strtoupper($request->input('currency','USD'));
        ...
        $booking->transaction_id    = $request->input('payment_intent_id','');
        $booking->booking_status    = 'confirmed';
        $booking->booking_reference = 'NM'.strtoupper(substr(md5(uniqid()),0,8));
        $booking->trip_type         = 'oneway';
        $booking->save();
        return response()->json(['success'=>true,'booking_id'=>$booking->id,'reference'=>$booking->booking_reference]);
    } catch (Exception $e) {
        \Log::error('Inline flight booking: '.$e->getMessage());
...
Route::get('/api/seat-map/{offer_id}', function($offer_id) {
    ...
    try {
        // First get seat maps for this offer
        $ch = curl_init();
        ...
        // Return first slice seat map cabins
