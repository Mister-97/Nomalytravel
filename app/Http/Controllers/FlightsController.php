...
class FlightsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $sort_by = $request->input('sort_by', 'best'); // Default sort by price
        ...
        if (isset($offerRequests['offers']) && is_array($offerRequests['offers'])) {
            ...
            // Sort offers: best, cheapest, fastest
            if ($sort_by === 'best' || $sort_by === '') {
                $offers = $offers->sortBy(function ($offer) {
                    $price = (float) ($offer['total_amount'] ?? 9999999);
                    $stops = count($offer['slices'][0]['segments'] ?? []) - 1;
                    $dur = 0;
                    $rawDur = $offer['slices'][0]['duration'] ?? '';
                    if ($rawDur) { preg_match('/PT(?:(\\d+)H)?(?:(\\d+)M)?/', $rawDur, $dm); $dur = (($dm[1] ?? 0) * 60) + ($dm[2] ?? 0); }
                    return $price + ($stops * 80) + ($dur * 0.3);
                });
            } elseif ($sort_by === 'price_asc') {
                $offers = $offers->sortBy('total_amount', SORT_REGULAR, false);
            } elseif ($sort_by === 'price_desc') {
                $offers = $offers->sortBy('total_amount', SORT_REGULAR, true);
            } elseif ($sort_by === 'duration_asc' || $sort_by === 'duration_desc') {
                $desc = $sort_by === 'duration_desc';
                $offers = $offers->sortBy(function ($offer) {
                    $rawDur = $offer['slices'][0]['duration'] ?? '';
                    if ($rawDur) { preg_match('/PT(?:(\\d+)H)?(?:(\\d+)M)?/', $rawDur, $dm); return (($dm[1] ?? 0) * 60) + ($dm[2] ?? 0); }
                    return 9999;
                }, SORT_REGULAR, $desc);
            } else {
    ...
    {
        try {
            Log::info('showBookingForm called', ['offer_id' => $offer_id]);

            // Try a fresh Duffel API call first.
            // If that fails (expired offer, network error), fall back to the session cache
            // that DuffelController stored when the search results page was rendered.
            $offer = null;
            try {
                ...
                Log::warning('getOffer failed, trying session cache: ' . $apiEx->getMessage(), ['offer_id' => $offer_id]);
            }

            // Session fallback — data cached by DuffelController when results page loaded
            if (!$offer) {
                $cachedOffers = session('duffel_search_offers', []);
                $offer = $cachedOffers[$offer_id] ?? session('offer');
                if ($offer) {
                    Log::info('Using session-cached offer', ['offer_id' => $offer_id]);
                }
            }

            if (!$offer) {
                Log::error('Offer not found in API or session cache', ['offer_id' => $offer_id]);
                return redirect()->route('flights.list')
                    ->with('error', 'This flight offer has expired. Please search again and select a new flight.');
            }

            // Persist offer in session for storeBooking fallback
            session(['offer' => json_decode(json_encode($offer), true)]);
            $countries = Country::whereNotNull('phonecode')->get();

            Log::info('Returning booking form view', ['offer_id' => $offer_id]);
            ...
            Log::error('showBookingForm unexpected error: ' . $e->getMessage());
            return redirect()->route('flights.list')->with('error', 'An unexpected error occurred. Please try again.');
        }
    }

    public function storeBooking(Request $request, $offer_id)
    {
        // Validation OUTSIDE the try block — ValidationException must propagate
        // naturally to Laravel so field-level error highlights work on the form.
        // Previously it was inside catch(Exception) which swallowed field errors
        // and, when LOG_CHANNEL was misconfigured, caused a 500.
        $validationRules = ["passengers" => "required|array"];
        ...
        try {
            ...
            if ($booking !== null && $booking->booking_status === "hold") {
                return back()->with("error", "This flight is already on hold. Please proceed to payment.");
            }
            ...
            // Fetch fresh offer from Duffel; fall back to session copy if expired.
            // Duffel offers can expire within minutes — the session copy (stored
            // when the booking form was first loaded) lets us proceed gracefully.
            try {
                $offer = $this->duffelService->getOffer($offer_id);
                ...
                if (!$offer) {
                    return back()->with("error",
                        "This flight offer has expired. Please search for flights again and select a new option.");
                }
                Log::warning("Duffel getOffer failed, using session offer: " . $fetchEx->getMessage(),
                    ["offer_id" => $offer_id]);
            }

            if (!$offer || isset($offer["error"])) {
                return back()->with("error", "The selected flight offer is no longer available. Please search again.");
            }
            ...
            $booking->flight_info = json_encode(session("offer"));

            // Match any selected ancillary services
            ...
            $selectedServicesTotal = array_sum(array_column($matchedServices, "total_amount"));
            ...
            $widgetData          = widget(29);
            $serviceFee          = (float) ($widgetData->extra_field_2 ?? 0);
            $servicePercent      = (float) ($widgetData->extra_field_3 ?? 0);
            $sub_total           = (float) ($offer["total_amount"] ?? 0);
            $totalServiceAmount  = $serviceFee + ($sub_total * $servicePercent / 100);
            ...
            $booking->checked_baggage     = json_encode($outboundSegment["passengers"][0]["baggages"] ?? null);
            $booking->carry_on_baggage    = json_encode($outboundSlice["baggages"]["carry_on"] ?? null);
            $booking->cabin_class         = $outboundSlice["cabin_class"]
                ?? ($outboundSegment["passengers"][0]["cabin_class_marketing_name"] ?? null);
            $booking->departure_time      = $outboundSegment["departing_at"] ?? null;
            $booking->arrival_time        = $outboundSegment["arriving_at"] ?? null;

            // Return segment (round-trip)
            if (count($offer["slices"]) > 1) {
                ...
                $booking->return_checked_baggage     = json_encode($returnSegment["passengers"][0]["baggages"] ?? null);
            ...
            // For non-hold bookings, immediately create the Duffel order while the offer is still fresh.
            // Offers expire in ~20 min; if we wait until paymentSuccess the offer will be gone.
            if ($request->booking_type != 'hold') {
                try {
                    $passengersForDuffel = [];
                    foreach ($validated['passengers'] as $passengerId => $p) {
                        $passengersForDuffel[] = array_merge($p, ['id' => $passengerId]);
                    }
                    $duffelOrder = $this->duffelService->createBooking(
                        $offer_id,
                        $passengersForDuffel,
                        [
                    ...
                    $booking->booking_reference = $duffelOrder['id'];
                    $booking->order_id          = $duffelOrder['id'];
                    $booking->save();
                    Log::info('Duffel order created in storeBooking', ['order_id' => $duffelOrder['id'], 'booking_id' => $booking->id]);
                } catch (\Exception $duffelEx) {
                    // Log but do NOT block the user — they can still proceed to payment;
                    // paymentSuccess will handle the failed order case gracefully.
                    Log::error('Duffel order creation failed in storeBooking: ' . $duffelEx->getMessage(), ['offer_id' => $offer_id]);
                }
            }

            return ($request->booking_type == "hold")
                ? redirect()->route("flight-booking")
                : redirect()->route("flights.payment", $booking->id);

        } catch (Exception $e) {
            Log::error("storeBooking error: " . $e->getMessage(), ["offer_id" => $offer_id]);
            return back()->with("error",
                "An error occurred while processing your booking. Please try again. (" . $e->getMessage() . ")");
        }
    }
    ...
    public function showPayment($booking_id)
    {
        ...
        if ($booking->booking_status == 'pending' && empty($booking->booking_reference)) {
            // Skip offer check when Duffel order already created in storeBooking.
            try {
                ...
                return back()->with('error', 'The selected flight offer has expired. Please search again.');
            }
        }
    ...
    public function processPaystackPayment(Request $request, $booking_id)
    {
        try {
            ...
            $psTxDetails = $booking->passenger_details;
            $psTxFst = is_array($psTxDetails) ? reset($psTxDetails) : [];
            $psTxEmail = $psTxFst['email'] ?? (optional(auth()->user())->email ?? 'noreply@nomalytravel.com');
    ...
    public function paystackCallback(Request $request, $booking_id)
    {
        try {
            ...
            if ($verification['success'] && $verification['status'] === 'success') {
                ...
                try {
                    $psDetails = $booking->passenger_details;
                    $psFst = is_array($psDetails) ? reset($psDetails) : [];
                    $psEmail = $psFst['email'] ?? null;
                    if ($psEmail) {
                        Mail::to($psEmail)->send(new FlightBookingMail($booking, $booking->user));
                    }
    ...
    public function paymentSuccess($booking_id)
    {
        ...
        try {
            ...
            // Verify Stripe payment actually succeeded before marking booking paid.
            if ($booking->payment_method === 'stripe' && $booking->transaction_id && Str::startsWith($booking->transaction_id, 'pi_')) {
                try {
                    $stripe = new \Stripe\StripeClient(config('services.stripe.secret'));
                    $paymentIntent = $stripe->paymentIntents->retrieve($booking->transaction_id);
                    if ($paymentIntent->status !== 'succeeded') {
                        \Log::warning('Stripe PaymentIntent not succeeded', ['pi' => $booking->transaction_id, 'status' => $paymentIntent->status]);
                        return redirect()->route('flights.payment', $booking->id)
                            ->with('error', 'Payment was not completed. Please try again.');
                    }
                } catch (\Exception $stripeEx) {
                    // Allow through — Stripe.js already confirmed on client side
                    \Log::error('Stripe verification error in paymentSuccess: ' . $stripeEx->getMessage());
                }
            }
            ...
            $booking->payment_status = 'paid';
            $booking->save();
            \Log::info('Booking confirmed/paid', ['booking_id' => $booking->id]);

            // Extract passenger email — works for guests and registered users
            $passengerDetails = $booking->passenger_details;
            $firstPassenger   = is_array($passengerDetails) ? reset($passengerDetails) : [];
            $passengerEmail   = $firstPassenger['email'] ?? null;
            $userObj          = $booking->user; // null for guests

            // Send confirmation emails
            try {
                if ($passengerEmail) {
                    Mail::to($passengerEmail)->send(new FlightBookingMail($booking, $userObj));
                }
                $adminEmail = widget(1)->extra_field_2;
                if ($adminEmail) {
                    Mail::to($adminEmail)->send(new FlightBookingMail($booking, $userObj));
                }
                \Log::info('Confirmation emails sent', ['booking_id' => $booking_id, 'passenger_email' => $passengerEmail]);
            } catch (\Exception $e) {
    ...
    public function book(Request $request)
    {
        try {
            ...
            $booking->user_id = auth()->id() ?? null; // Assuming user is authenticated
