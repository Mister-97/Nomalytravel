...
class HotelsController extends Controller
{
    ...
    public function __construct(PaystackService $paystackService, HotelbedsService $hotelbedsService)
    {
    ...
    public function index(Request $request){
        ...
        // Hotelbeds live search
        $hbParams = [];
        if ($request->filled('hb_city'))     $hbParams['city']      = $request->hb_city;
        if ($request->filled('hb_checkin'))  $hbParams['check_in']  = $request->hb_checkin;
        if ($request->filled('hb_checkout')) $hbParams['check_out'] = $request->hb_checkout;
        if ($request->filled('hb_adults'))   $hbParams['adults']    = $request->hb_adults;
        if ($request->filled('hb_rooms'))    $hbParams['rooms']     = $request->hb_rooms;
        if ($request->filled('min_price'))   $hbParams['min_price'] = $request->min_price;
        if ($request->filled('max_price'))   $hbParams['max_price'] = $request->max_price;

        if (!empty($hbParams)) {
            $hbResult = $this->hotelbedsService->searchHotels($hbParams, 18);
        } else {
            $hbResult = ['hotels' => [], 'error' => null, 'total' => 0];
        }
        $arr['hbHotels'] = $hbResult['hotels'];
        $arr['hbError']  = $hbResult['error'];
        $arr['hbTotal']  = $hbResult['total'] ?? 0;
        $arr['hbHasKey'] = $this->hotelbedsService->hasKey();
        $arr['hbCities'] = $this->hotelbedsService->getCityOptions();
    ...
    public function processStripePayment(Request $request, $booking_id)
    {
        try {
            ...
            // Check if the booking belongs to the authenticated user
            // Allow when user_id is null (guest booking) and visitor is also a guest
            if ($booking->user_id !== null && $booking->user_id !== auth()->id()) {
                abort(403);
            }
    ...
    public function processPaystackPayment(Request $request, $booking_id)
    {
        try {
            ...
            $transactionData = [
                'email' => optional(auth()->user())->email ?? optional($booking->user)->email ?? 'guest@nomalytravel.com',
                'amount' => $ngnAmount,
